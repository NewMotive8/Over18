...
export interface EconomySection {
  ...
  manages: string;
  backend: { status: 'available' } | { status: 'pending'; missing: string[] };
}

/** What every editing screen still needs from the server. */
const NO_WRITE_WORKFLOW =
  'Draft versions cannot be scheduled, published or cancelled from the admin: that workflow, its validation and its audit record do not exist on the server yet.';
...
export const ECONOMY_SECTIONS: readonly EconomySection[] = [
  ...
  },
  {
    ...
    backend: { status: 'pending', missing: ['No admin endpoint lists, creates or edits plan versions.', NO_WRITE_WORKFLOW] },
  },
  {
    key: 'packs',
    label: 'Credit packs',
    ...
    backend: { status: 'pending', missing: ['No admin endpoint lists, creates or edits pack versions.', NO_WRITE_WORKFLOW] },
  },
  {
    key: 'action-costs',
    label: 'Action costs',
    ...
    backend: { status: 'pending', missing: ['No admin endpoint lists, creates or edits a ruleset or its action costs.', NO_WRITE_WORKFLOW] },
  },
  {
    key: 'allowances',
    label: 'Allowances',
    ...
    backend: {
      status: 'pending',
      missing: ['No admin endpoint lists, creates or edits ruleset allowances, and the preview does not report them.', NO_WRITE_WORKFLOW],
    },
  },
  {
    key: 'rewards',
    ...
    backend: {
      status: 'pending',
      missing: ['No admin endpoint lists, creates or edits ruleset rewards, and the preview does not report them.', NO_WRITE_WORKFLOW],
    },
  },
  {
    key: 'versions',
    ...
    backend: {
      status: 'pending',
      missing: [
        'No admin endpoint lists versions by state or returns their effective and publication timestamps.',
        'No admin endpoint validates, schedules, publishes or cancels a version.',
      ],
    },
  },
];
