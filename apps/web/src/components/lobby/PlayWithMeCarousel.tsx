...
import { adultAgeFromBand, personaTags } from '../../lib/lobbyContent';
...
function PlayWithMeCard({ character, index }: { character: PublicCharacter; index: number }) {
  ...
  const tags = personaTags(character, index).slice(0, 2);
 ...
 */
export default function PlayWithMeCarousel({ characters }: { characters: PublicCharacter[] }) {
    ...
    <section aria-label="Play with me" className="flex flex-col gap-3">
      ...
      <div className="flex snap-x gap-3 overflow-x-auto px-4 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {characters.map((character, index) => (
          <PlayWithMeCard key={character.id} character={character} index={index} />
        ))}
      </div>
    </section>
  );
