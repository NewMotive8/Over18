...
## Character visual identity foundation (US-16A)

Swipey owns each character's **visual identity**: a versioned **Visual DNA** record (`character_visual_identities`) plus a lifecycle-managed set of **visual assets** (`character_visual_assets`). Visual DNA describes *identity only* (face, eyes, hair, skin, body, distinctive features, adult apparent-age band); *presentation* (clothing, pose, lighting, camera, …) is a generation-time concern and is deliberately rejected from Visual DNA — this identity-vs-presentation separation is a core principle. Each character can hold multiple identity versions; exactly one is `active` (enforced by a partial unique index), and redesigns create a new version while retiring the old one (never overwritten — provenance and rollback). An asset is **canonical** only when `kind='reference' AND status='approved' AND is_canonical=true`; a generated asset never auto-promotes — canonical status is reachable only through an explicit approval transition that records `approved_by`/`approved_at`. `provenance` is server-side-only internal metadata (never on the wire), and `storage_key` is a reserved forward-compatible field — US-16A moves no image bytes.
